...
description: "将微信公众号文章导出为 Markdown 格式。当用户提供微信公众号链接 (mp.weixin.qq.com) 或要求下载/导出/保存微信文章时触发。默认保存到 /root/openclaw-content 目录。"
---
...
**基础目录**：`~/.openclaw/workspace/skills/wechat-article-reader`
...
> **最快方式：直接运行 `read-wechat "URL"` 即可，无需手动执行以下步骤。**
...
# 运行导出脚本
python3 ~/.openclaw/workspace/skills/wechat-article-reader/scripts/export.py "$URL" "$OUTPUT_DIR"
```
...
```bash
# 基本导出
python3 ~/.openclaw/workspace/skills/wechat-article-reader/scripts/export.py "https://mp.weixin.qq.com/s/xxx" /root/openclaw-content

# 指定自定义输出目录
python3 ~/.openclaw/workspace/skills/wechat-article-reader/scripts/export.py "$URL" "/path/to/output"
```
