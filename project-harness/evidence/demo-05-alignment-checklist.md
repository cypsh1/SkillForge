...
### Batch 4：B 类决策执行 ✅ 已完成 2026-04-07
- B1：exec 字段前缀 `f-s-*` → `f-x-*` ✅ 已执行（editor-panel 1 处 + inspector-panel 2 处）
- D4-D6：确认为主应用增量能力，保留不对齐，已记录在 D 类表中

### Batch 5：运行时确认项 + 产品决策 ✅ 已完成 2026-04-07
- C4 scroll-behavior：JS 已用 `behavior:"smooth"`，功能等价 → 不修
- C5 tooltip 偏移 +12 vs +14：2px 不可察觉 → 不修
- C6 tooltip 文案："点击跳转" → "点击双面板跳转" ✅ 已修复
- C7/C8 hover 背景：`color-mix(foreground 3%)` ≈ `rgba(255,255,255,.03)` 功能等价 → 不修
- C9 bub-i 字号 10px vs 11px：气泡临时弹出 1px 不可察觉 → 不修
- P1 左进度条颜色：**决定保留主应用区块跟随色**（理由：与底部上下文栏色点/区块名一致；三栏布局中信息密度更高，跟随色传达上下文归属）
